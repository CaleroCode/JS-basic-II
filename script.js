//Bienvenido a Javascript básico parte II, donde veremos condicionales y bucles.
...
const postres = ["helado", "tarta", "pastel"] // Postres
console.log(postres);
...
const pet = "perro";
    if (const === perro);
        ...
        console.log("No tengo una mascota de la lista");
    }
...
const result = weather === "soleado" ? "Me vestiré con pantalón" : "Me vestiré con pantalón";
...
for (const index in programmers) {
    const p = programmers[index];
    console.log(`${index}: ${p.name} ${p.lastname}, su aporte fue ${p.knowledge}`);
}
...
console.log(`Área 1: ${calcularAreaRectangulo(5, 10)}`);
console.log(`Área 2: ${calcularAreaRectangulo(7, 3)}`);
console.log(`Área 3: ${calcularAreaRectangulo(12, 4.5)}`);